...
def validate_settings() -> bool:
    ...
    if not settings.GROQ_MODEL or settings.GROQ_MODEL.strip() == "":
        logger.warning(
            "GROQ_MODEL is not set. Defaulting to 'openai/gpt-oss-20b'. "
            "Set GROQ_MODEL in your .env to override."
        )
        settings.GROQ_MODEL = "openai/gpt-oss-20b"
    elif settings.GROQ_MODEL in ("llama-3.1-8b-instant", "llama3-8b-8192", "llama-3-8b-8192"):
        logger.warning(
            "GROQ_MODEL '%s' is decommissioned/deprecated on Groq. Upgrading to 'openai/gpt-oss-20b'.",
            settings.GROQ_MODEL,
        )
        settings.GROQ_MODEL = "openai/gpt-oss-20b"
